...
/// \~japanese
/// CoineyKit のログイン状態をチェックし、 `aCompletionBlock` を実行します。
/// `aParentViewController` が `nil` でなく、且つ未ログインなら、ログイン画面を表示します。
/// \param aCompletionBlock
///   \~english The block to execute upon completion
///   \~japanese ログイン状態チェック後に実行するブロック
/// \param aParentViewController
///   \~english Parent of login dialog
///   \~japanese 未ログイン状態なら、この上にログイン画面を表示
