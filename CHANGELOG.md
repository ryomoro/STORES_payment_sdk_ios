...
- Xcode13に対応しました。
  - Xcode13以降のバージョンでビルドする場合は、本バージョン以降をご利用ください。

- Xcode13で本SDKを使用した際に iOS 15/ iPadOS 15 以降でナビゲーションバーの色が透明などになる不具合を修正しました。 
...
- セミセルフレジに対応しました。
  - セミセルフレジとしてCoineyKitを利用する場合は、 `prepareForSemiSelfCheckoutModeInController` メソッドを使用してください。 

- 決済完了画面および取引詳細画面のレシートメール送信TextFiledの表示 / 非表示を設定できるようにしました。
  - デフォルトは **非表示**です。
  - 表示するには `CYKit.setEnableSendReceipt(true)`を呼んでください。
...
- iOS14.2で起きていたログイン画面の不具合を修正しました
	- キーボード表示が遅延する不具合
	- パスワード入力欄でフリック・タップ入力が反応しない不具合
- サービス名称変更しました。
  - UI上の表記を「STORES（ストアーズ）ターミナル」 のサービス名称を、「STORES（ストアーズ）決済」に変更しました。(Frameworkの名称は変更しておりません。)

## 6.5.1 (2020-09-23)
* iOS13でスワイプで決済画面を閉じた際にデリゲートメソッドが呼ばれない不具合を修正しました。
* CYLookUpTransaction() で電子マネー決済の取引情報を取得できない場合がある不具合を修正しました。

## 6.4.1 (2020-07-07)
* 電子マネー決済に対応しました。
* サービス名称変更しました。
  * UI上の表記を「Coiney（コイニー）」のサービス名称を、「STORES（ストアーズ）ターミナル」に変更しました。(Frameworkの名称は変更しておりません。)

## 5.3.9 (2020-03-11)
* 売上取消・返品画面をスワイプで閉じないように対応しました。

## 5.3.8 (2020-03-03)
* Xcode11に対応しました。

Xcode11以降のバージョンでビルドする場合は、本バージョン以降をご利用ください。

## 5.3.7 (2019-10-24)
* iOS 13 でログインに時間がかかる問題を対応しました。
